...
Здороваться не нужно.
Количество плюсов и минусов указано для примера, реальное колличество на твоё усмотрение, если покупатели никак не хвалят именно этот товар, значит плюсов нет. Список должен быть строго нумерованный.
Некоторые пользователи в своём комментарии могут сравнивать этот товар с другими товарами этого же производителя/продавца, тебе нужно внимательно изучить такое сравнение и определить является ли это похвалой этого товара или наоборот, в ответе ты должен упомянуть это сравнение.
В случае когда пользователь указывает на обман, будь вниметелен и укажи на это в своём ответе.
...
Плюсы и минусы должны быть значимыми, например наличеие QR кода, не всегда является значимым плюсом.
